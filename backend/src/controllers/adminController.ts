...
import { User, Profile, Project, ProjectMember, Task, Client, ClientFeedback, ActivityLog } from '../models/index.js';
import { AuthenticatedRequest } from '../middleware/auth.js';
import { hashPassword } from '../utils/auth.js';
import { successResponse, errorResponse } from '../utils/response.js';
import { CreateUserRequest, UpdateUserRequest, IUser, IProfile, AuthPayload } from '../types/index.js';
...
export const createUser = async (
  ...
  try {
    ...
    const { email, password, fullName, role = 'team_member' }: CreateUserRequest & { role?: string } = req.body;
...
export const updateUser = async (
  ...
  try {
    ...
    const { fullName, email, role, status }: UpdateUserRequest & { role?: string; status?: 'active' | 'inactive' | 'suspended' } = req.body;
...
export const resetUserPassword = async (
  ...
  try {
    ...
    emailService.sendPasswordResetEmail((profile.user as any).email, profile.fullName, 'admin-reset').catch((error: any) => {
...
export const createProject = async (
  ...
  try {
    ...
    if (req.body.teamMembers && Array.isArray(req.body.teamMembers)) {
      const projectMembers = req.body.teamMembers.map((userId: string) => ({
        project: project.id,
        user: userId,
        role: 'member'
      }));
...
export const updateProject = async (
  ...
  try {
    ...
    if (updates.teamMembers && Array.isArray(updates.teamMembers)) {
      ...
      // Add new members
      const projectMembers = updates.teamMembers.map((userId: string) => ({
        project: projectId,
        user: userId,
        role: 'member'
      }));
...
export const updateAdminPrivileges = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
...
export const getAdminPrivileges = async (
  req: AuthenticatedRequest,
  res: Response,
  next: NextFunction
): Promise<void> => {
  try {
    ...
    if (!user.profile || (user.profile as any).role !== 'admin') {
      errorResponse(res, 'User is not an admin', undefined, 400);
      return;
    }
