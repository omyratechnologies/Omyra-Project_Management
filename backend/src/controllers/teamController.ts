    ...
    if (projectId) {
      ...
      if (req.user.role === 'admin') {
        ...
        profiles = profilesData.map(profile => ({
          ...profile.toObject(),
          userId: (profile.user as any)._id || (profile.user as any).id,
        ...
        if (userProjectIds.length === 0) {
          ...
          projectMembers.forEach(pm => {
            const userId = (pm.user as any)._id.toString();
            if (!uniqueProfiles.has(userId)) {
              uniqueProfiles.set(userId, {
                ...(pm.user as any).profile.toObject(),
                userId: (pm.user as any)._id,
                email: (pm.user as any).email
              });
            }
          });
...
export const getTeamMember = async (
  ...
  try {
    ...
    const targetUserId = (profile.user as any).id.toString();
