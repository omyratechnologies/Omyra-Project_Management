...
// Hook for task issues count
export const useTaskIssuesCount = (taskId: string) => {
  return useQuery({
    queryKey: ['task-issues-count', taskId],
    queryFn: async () => {
      try {
        const issues = await apiClient.getTaskIssues(taskId);
        return issues.length;
      } catch (error) {
        console.error('Failed to fetch task issues count:', error);
        return 0;
      }
    },
    enabled: !!taskId,
